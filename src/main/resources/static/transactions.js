const cargarTransacciones = async() => {
	try {
		let res = await fetch("/api/transactions/getAll/",{
			method : 'GET',
			headers : {
		...
		if(res.status === 200){
			...
			$(document).ready(function() {
				...
				$('#example tbody').on( 'click', 'tr', function () {
					...
					alert("¿Estás seguro de que deseas borrar esta transacción?");
					borrarTransaccion(table.row( this ).data());
				} );
...
async function borrarTransaccion(tr){
	try{
	
	let res = await fetch("/api/transactions/delete/" + tr.id,{
		method : 'DELETE'
	});

	if(res.status === 200){
		alert("Transaccion borrada correctamente");
		location.reload();
	} else if(respuesta.status === 404){
